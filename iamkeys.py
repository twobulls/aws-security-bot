...
def iamkeys (commandargs):
	...
	for user in iam.users.all():
		...
		user_name = user.name
		bullkit.debug('Checking the access keys of: {}'.format(user_name), commandargs)
		for access_key in iam.User(user_name).access_keys.all():
			...
			if access_key_age >= access_key_warn_age and access_key_age < access_key_expire_age and access_key.status == 'Active':
				...
				if not keys_to_warn.get(user_name):
					keys_to_warn[user_name] = []
				key_approaching_expiration = {'id': access_key_id, 'time left': access_key_expire_age - access_key_age}
				keys_to_warn[user_name].append(key_approaching_expiration)
			...
			if access_key_age >= access_key_expire_age and access_key.status == 'Active':
				...
				if not expired_keys.get(user_name):
					expired_keys[user_name] = []
				expired_keys[user_name].append(access_key_id)
	...
	# If we found access keys that are approaching expiration or have expired...
		...
		if keys_to_warn:
			...
			for user, access_key in keys_to_warn.items():
				...
				keys_to_warn_list.append('{}: {} (expires in {} days)'.format(user, access_key_id, access_key_days_left))
		...
		if expired_keys:
			...
			for user, access_key in expired_keys.items():
				expired_key = '{}: {}'.format(user, access_key)
